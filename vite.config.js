...
function cctvProxy() {
  ...
  return {
    name: 'cctv-proxy',
    configureServer(server) {
      server.httpServer?.on('close', () => { remuxer.shutdown(); puller.shutdown(); });
      server.middlewares.use('/api/cctv', async (req, res) => {
        try {
          ...
          if (url.pathname.startsWith('/media/')) {
            ...
            // HLS feeds: serve from a proxy-owned segment store with a
            // proxy-generated playlist. HTTP(S) .m3u8 upstreams use the Node
            // puller (createHlsPuller, no ffmpeg); other stream kinds use the
            // ffmpeg segment producer (createHlsRemuxer). Falls through to the
            // direct proxy if neither applies.
            const hlsStrategy = (feedType === 'hls' && mediaUrl && /^(https?|rtmps?|rtmpt):\/\//i.test(mediaUrl))
              ? (/\.m3u8(\?|$)/i.test(mediaUrl) ? puller : (remuxer.isAvailable() ? remuxer : null))
              : null;
            if (hlsStrategy) {
              ...
              if (isPlaylist) {
                ...
                res.writeHead(200, {
                  ...
                  'Cache-Control': 'no-store',
                  'X-CCTV-Source': hlsStrategy === puller ? 'hls-pull' : 'ffmpeg-remux',
                });
                res.end(playlist);
                return;
              }
